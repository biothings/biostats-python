...
class BioThingsPanel extends React.Component {
  ...
  render() {
    return (
      <section className="margin0Auto padding20 centerText mB-back">
        <img src="img/biothings-text.png" width="300px" className="margin20"/>
        <button style={{position:'absolute', right:'20px'}} disabled={this.props.user.name ? false : true} className={"btn " + (this.props.user.name ? 'btn-yellow' : 'btn-grey') } onClick={this.fetchRealTimeData}>Refresh</button>
